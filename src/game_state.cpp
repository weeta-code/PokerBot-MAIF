...
Player::Player(string _id, double _stack, bool _is_human) 
    :   id(_id), stack(_stack), current_bet(0), is_folded(false), 
        is_all_in(false), is_human(_is_human),
        times_folded(0), times_raised(0), times_called(0), hands_played(0) {
...
GameState::GameState(RiskProfiler* rp, OddsCalculator* oc) 
    : risk_profiler(rp), odds_calculator(oc) {
        pot_size = 0;
...
void GameState::next_street() {
    ...
    current_player_index = (dealer_index + 1) % num_players;
    while(players[current_player_index].is_folded || players[current_player_index].is_all_in) {
        current_player_index = (current_player_index + 1) % num_players;
    }

    // move to next stage
    if (stage == Stage::PREFLOP) stage = Stage::FLOP;
    else if (stage == Stage::FLOP) stage = Stage::TURN;
    else if (stage == Stage::TURN) stage = Stage::RIVER;
    else if (stage == Stage::RIVER) stage = Stage::SHOWDOWN;
...
bool GameState::record_action(int player_idx, string action, double amount) {
    ...
    if (action == "call") {
        ...
        if (call_amount > p.stack) call_amount = p.stack; // all-in call
...
//             active++;
//             if (p.current_bet == current_street_highest_bet && current_street_highest_bet > 0) {
//                 matches++;
//             }
...
void GameState::resolve_winner() {
    ...
    // here we use odds_calculator to evaluate hands
    // Prolly, iterate all non-folded players, run evaluate_hand
    // find max/best hand
    // hand pot to winner
    
    // placeholder for later integration
    cout << "TODO: find winner using odds_calculator to evaluate hands \n";
